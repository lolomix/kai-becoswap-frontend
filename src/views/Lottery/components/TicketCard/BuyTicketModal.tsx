...
import { Button, Modal } from '@becoswap-libs/kai-uikit'
  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      <TicketInput
        ...
        max={fullBalance}
        symbol="TICKET"
        availableSymbol="BECO"
      />
      <div>
        <Tips>{t(`1 Ticket = ${LOTTERY_TICKET_PRICE} BECO`, { num: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>
          {t('Ticket purchases are final. Your BECO cannot be returned to you after buying tickets.')}
        </Announce>
        <Final>{t('You will spend: %amount% BECO', { amount: cakeCosts(val) })}</Final>
      </div>
      <ModalActions>
